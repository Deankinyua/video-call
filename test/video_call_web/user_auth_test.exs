defmodule VideoCallWeb.UserAuthTest do
  ...
  @remember_me_cookie "_video_call_web_user_remember_me"

  setup %{conn: conn} do
    conn =
      conn
      |> Map.replace!(:secret_key_base, VideoCallWeb.Endpoint.config(:secret_key_base))
      |> init_test_session(%{})
  ...
  describe "log_in_user/3" do
    test "stores the user token in the session", %{conn: conn, user: user} do
      ...
      assert redirected_to(conn) == ~p"/call"
  ...
  describe "logout_user/1" do
    ...
    test "broadcasts to the given live_socket_id", %{conn: conn} do
      ...
      VideoCallWeb.Endpoint.subscribe(live_socket_id)
  ...
  describe "on_mount :ensure_authenticated" do
    ...
    test "redirects to home page if there isn't a valid user_token", %{conn: conn} do
      user_token = "invalid_token"
    ...
    test "redirects to home page if there isn't a user_token", %{conn: conn} do
      session = get_session(conn)
  ...
  describe "on_mount :redirect_if_user_is_authenticated" do
    test "redirects to the call page if there is an authenticated  user ", %{
      conn: conn,
      user: user
    } do
      user_token = Accounts.generate_user_session_token(user)
  ...
  describe "redirect_if_user_is_authenticated/2" do
    test "redirects to the call page if user is authenticated", %{conn: conn, user: user} do
      conn =
        conn
